...
module.exports = {
    ...
    sendCode: async function sendCode(email, firstName, lastName, url, confirmationCode, host) {
        fs.readFile('./app_server/views/confirmationEmail.hbs', 'UTF-8', function(err, data) {
            if (err) {
                console.log(err);
            }
            else {
                data = data.replace('{{FIRSTNAME}}', firstName).replace('{{LASTNAME}}', lastName).replace('{{CODE}}', confirmationCode).replace('{{LINK}}', 'http://' + host + '/confirmation/' + url + '/' + confirmationCode);
                sendEmail(email, 'Confirmation code', data);    
            }
        });
    },
    sendResetPassword: async function sendResetPassword(email, firstName, lastName, url, host) {
        fs.readFile('./app_server/views/resetPasswordEmail.hbs', 'UTF-8', function(err, data) {
            if (err) {
                console.log(err);
            }
            else {
                data = data.replace('{{FIRSTNAME}}', firstName).replace('{{LASTNAME}}', lastName).replace('{{LINK}}', 'http://' + host + '/passwordReset/' + url);
